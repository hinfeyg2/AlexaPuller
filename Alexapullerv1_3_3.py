...
class PullFiles(threading.Thread):
	...
	def run(self):
		...
		report.write("Dogs are really cool")
		selectedList = []
		...
		for i in selectedList:
			if i in frame.firstsourcelist:
				selectedfirstsource.append(i)
			else:
				pass
				
		if frame.twoDirFilename != "":
			for x in selectedList:
				if x in frame.secondsourcelist:
		...
		self.listlength = len(selectedfirstsource)
		
		
		for r, d, f in os.walk(frame.DirFilename):
			for file in f:
				f_name, f_ext = os.path.splitext(file)
				if ".mov" == f_ext:
		...
		totallistlength = self.listlength + len(selectedsecondcource)
		if frame.twoDirFilename != "":
			for r, d, f in os.walk(frame.twoDirFilename):
				for file in f:
					f_name, f_ext = os.path.splitext(file)
					if ".mov" == f_ext:
						if f_name in selectedsecondcource:
							src_abs_path = os.path.join(r, file)
							src_relative_path = os.path.relpath(src_abs_path, frame.twoDirFilename)
							dst_abs_path = os.path.join(frame.DirDest, src_relative_path)
							dst_dir = os.path.dirname(dst_abs_path)
							if not os.path.exists(dst_dir):
								os.makedirs(dst_dir)
...
app = wx.App(True)
frame = MyFrame(None,"Alexa Puller v1.3.2")
app.MainLoop()
